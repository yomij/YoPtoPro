...
// 显示选择提示窗
function showHelper(config: DeployConfig[]): Promise<{ value: DeployConfig | undefined }> {
...
// 初始化helper
function initHelper(config: DeployConfig[]) {
...
// 查找符合条件的配置项
function findInfoByName(config: DeployConfig[], name: string) {
  for (let item of config) {
    if (item.name === name) {
      return item;
    }
